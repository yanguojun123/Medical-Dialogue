...
def contras_simple_loss(input1, input2, eps):
    pdist = nn.PairwiseDistance(p=2)

    dis = pdist(input1, input2)
    print(dis)
    try:
        # if dis.item() > thresh:
        #     return dis
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def forward(
        ...
        #encoder_hidden_states1 = hidden_states[0::2, 0, :]
        #encoder_hidden_states2 = hidden_states[1::2, 0, :]
        #decoder_hidden_states1 = torch.mean(sequence_output[0::2, :, :], dim=1)
        #decoder_hidden_states2 = torch.mean(sequence_output[1::2, :, :], dim=1)
        decoder_hidden_states1 = sequence_output[0::2, 0, :]
        decoder_hidden_states2 = sequence_output[1::2, 0, :]
        #print("encoder_hidden_state1:", encoder_hidden_states1.size(), encoder_hidden_states1.device)
        #print("encoder_hidden_state2:", encoder_hidden_states2.size(), encoder_hidden_states1.device)
        ...
        if labels is not None:
            ...
            labels_embed = self.shared(labels)
            labels_embed = torch.mean(labels_embed, dim=1)
            #print("labels_embed:", labels_embed.size())
            ...
            #loss2 = contras_simple_loss(encoder_hidden_states1, encoder_hidden_states2, 2)
            print("decoder_hidden_states", decoder_hidden_states1.size(), decoder_hidden_states2.size())
            loss_con = ContrastiveLoss(decoder_hidden_states1.size(0))
            #loss2 = loss_con(encoder_hidden_states1, encoder_hidden_states2)
            loss3 = loss_con(decoder_hidden_states1, decoder_hidden_states2)
