...
class Con_mt5(MT5ForConditionalGeneration):
    def __init__(self, config, args, mixture, device):
        super(Con_mt5, self).__init__(config)
        self.args = args
    ...
    def forward(self, input_ids=None,
        ...
        return_dict=None,
        parameter_id= None):
        """For each batch, the target language is fixed, either language1 or language2."""
        ...
        print("sequence:", sequence_output.size())
        ...
        decoder_hidden_states1 = sequence_output[0::2]
        decoder_hidden_states2 = sequence_output[1::2]
        ...
        if labels is not None:
            ...
            loss_con = ContrastiveLossELI5(batch_size=sequence_output.size(0), temperature=0.1)
            loss = loss_fct(lm_logits.view(-1, lm_logits.size(-1)), labels.view(-1)) + loss_con(decoder_hidden_states1, decoder_hidden_states2)
            # print("lm:",torch.argmax(F.softmax(lm_logits.view(-1, lm_logits.size(-1)),dim=-1),dim=-1))
